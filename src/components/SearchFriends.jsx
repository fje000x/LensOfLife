    ...
    try {
      ...
      const results = await Promise.all(querySnapshot.docs.map(async (doc) => {
        ...
        return {
          uid: doc.id,
          ...doc.data(),
          isAdded, // This flag will be true if the user has already been added as a friend
        };
      }));
  ...
  const handleAddFriend = async (friendUid, friendUsername) => {
    ...
    try {
      await setDoc(doc(db, 'users', currentUser.uid, 'friends', friendUid), {
        uid: friendUid,
        username: friendUsername, // Add the username to the friend document
        addedOn: serverTimestamp(),
      });
      console.log('Friend added successfully!');
  
      // Update the isAdded flag for the user
